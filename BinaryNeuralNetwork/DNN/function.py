...
class BinaryLinearFunction(Function):
    ...
    def forward(ctx, input, weight, bias=None):
        ctx.save_for_backward(input, weight, bias)  ## save for backward
        weight_b = where(weight>=0, 1, -1)  ## Deterministic
        output = input.mm(weight_b.t())  #mm mutiple 
        if bias is not None:
...
class BinaryStraightThroughFunction(Function):  ## no bias
    @staticmethod
