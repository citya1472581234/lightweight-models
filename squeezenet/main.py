...
parser.add_argument('--learning-rate', type=float, default=0.003, metavar='LR', help='learning rate')
...
def train(epoch):
    ...
    train_accuracy = int(correct) / float(len(train_loader.dataset))
...
def val():
    ...
    print('val  _correct',correct)
    val_accuracy = int(correct) /(73.0*64.0)
...
def test():
    ...
    return 100.0 * int(test_correct) / (float(total_examples))
