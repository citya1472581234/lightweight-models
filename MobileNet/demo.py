...
def train_epoch(model, loader, optimizer, epoch, n_epochs, print_freq=1):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if batch_idx % print_freq == 0:
            ...
            print(res)
            loss_tr.append(losses.avg)
            loss_val.append(losses.val)
            tr_error.append((error.avg)*100)
            val_error.append((error.val)*100)
...
def train(model, train_set, test_set, save, n_epochs=200, valid_size=5000,
    ...
    # Start log
    with open(os.path.join(save, 'results.csv'), 'w') as f:   # w = write 
...
def demo(data, save, efficient=True, valid_size=5000,n_epochs=30, batch_size=64, seed=None):
    """
